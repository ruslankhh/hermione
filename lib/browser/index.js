...
module.exports = class Browser {
    ...
    _createSession() {
        ...
        this._addCommands(session);

        return session;
    }

    _addCommands(session) {
        this._addMetaAccessCommands(session);
        this._addExtendOptionsMethod(session);
        this._decorateUrlMethod(session);
        this._decorateWindowHandleSizeMethod(session);

        const commandsList = ['assert-view'];
        commandsList.forEach((command) => require(path.resolve(__dirname, 'commands', command))(this));
    }
    ...
    getScreenshotPath(stateName) {
        const test = utils.getTestContext(this._session.executionContext);
        const screenshotDir = this._config.screenshotDir;

        if (_.isFunction(screenshotDir)) {
            return path.resolve(process.cwd(), screenshotDir(test), `${stateName}.png`);
        }

        return path.resolve(
            process.cwd(),
            screenshotDir,
            test.hashedFullTitle(),
            this.id,
            `${stateName}.png`
        );
    }
