...
const {logger, getTestContext} = require('../utils');
...
module.exports = class Browser {
    ...
    constructor(config, id) {
        ...
        this._systemConfig = config.system;
    ...
    _addCommands() {
        ...
        commandsList.forEach((command) => require(`${__dirname}/commands/${command}`)(this, this._systemConfig));
    }
    ...
    getScreenshotPath(stateName) {
        const test = getTestContext(this._session.executionContext);
        const state = `${stateName}.png`;
        const {screenshotsDir} = this._config;

        return _.isFunction(screenshotsDir)
            ? path.resolve(process.cwd(), screenshotsDir(test), state)
            : path.resolve(process.cwd(), screenshotsDir, test.hashedFullTitle(), this.id, state);
    }
